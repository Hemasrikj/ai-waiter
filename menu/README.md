...
| `accessor.py` | Python module — loads and renders `menu.json` for the AI waiter |
| `__init__.py` | Re-exports `MENU` and `_render_menu` as the public `menu` package API |
| `test_accessor.py` | Manual smoke-test — run directly to print the rendered menu |
...
## Smoke-testing (`test_accessor.py`)

`test_accessor.py` is a lightweight manual check — run it directly from the `menu/` directory to verify the module loads and renders without errors:

```bash
cd menu
python test_accessor.py
```

Expected output is the full formatted menu printed to stdout. No test framework is required.
...
```bash
python3 menu/test_accessor.py > menu/menu_for_system_prompt.txt
```
