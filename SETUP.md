...
├── menu/
│   ├── accessor.py       # Runtime module: MENU dict + _render_menu()
│   ├── __init__.py       # Re-exports MENU and _render_menu as the menu package
│   ├── test_accessor.py  # Smoke-test: run directly to print the rendered menu
│   ├── menu.json         # Normalised menu data (generated by convert.py)
│   ├── menu_for_system_prompt.txt  # Pre-rendered menu text for the system prompt
│   ├── scanned-menu.json # Raw source menu (edit this to change the menu)
│   ├── convert.py        # Converts scanned-menu.json → menu.json (Python)
│   ├── convert.js        # Same conversion in Node.js
...
If you edit `menu/scanned-menu.json`, regenerate the derived files:

```bash
# 1. Rebuild menu.json
uv run python menu/convert.py

# 2. Rebuild the system prompt text snapshot
...
| `langchain-ollama` | Local Ollama model integration (Gemma) |
